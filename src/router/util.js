...
export function updateUrl (url) {
    ...
    // let host = encodeURIComponent(window.location.href.split('#')[0])
    // console.log("encodeURIComponent---url---",encodeURIComponent(window.location.href.split('#')[0]))
    console.log("host---",host)
    // 如果是安卓用户，每次存最新的
    if (system == 1) {
      console.log('安卓用户！！！---',host)
      ...
      console.log('ios用户！！！---',sdkUrl)
      if (!sdkUrl) {
        console.log('ios用户没存地址，先存地址---',host)
        store.dispatch('set_sdk_url', host)
      }
    }
}
